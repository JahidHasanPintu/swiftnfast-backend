...
export class EditCustomerService {
  ...
  async updateCustomerInfo(
    customerIdOrId: string,
    updateData: EditCustomerDto,
    ...
    if (!existingCustomer) {
      existingCustomer = await this.customerModel.findOne({
        customerId: customerIdOrId,
      });
    }

    if (!existingCustomer) {
      throw new NotFoundException(
        `Customer with ID ${customerIdOrId} not found`,
      );
    }
    ...
    if (updateFields.customerName !== undefined)
      setFields.customerName = updateFields.customerName;
    if (updateFields.contactNumber !== undefined)
      setFields.contactNumber = updateFields.contactNumber;
