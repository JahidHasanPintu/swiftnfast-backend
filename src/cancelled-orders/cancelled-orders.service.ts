...
export class CancelledOrdersService {
  constructor(
    @InjectModel('CancelledOrder')
    private cancelOrdersModel: Model<CancelledOrderDocument>,
  ) {}

  async createCancelOrders(
    cancelledOrdersDto: CancelledOrdersDto,
  ): Promise<CancelledOrderDocument> {
    const createCancelOrders = new this.cancelOrdersModel(cancelledOrdersDto);
    return createCancelOrders.save();
  }

  async createBulkCancelOrders(
    bulkCancelledOrdersDto: CancelledOrdersDto[],
  ): Promise<CancelledOrderDocument[]> {
    // Handle bulk cancellation logic here
    const createdOrders = await Promise.all(
      bulkCancelledOrdersDto.map((data) => this.createCancelOrders(data)),
    );
