...
export class CancelledOrdersController {
  ...
  @Post('bulk-cancel')
  async createBulkCancel(
    @Body() bulkCancelledOrdersDto: BulkCancelledOrdersDto,
  ) {
    // Handle bulk cancellation request
    return this.cancelledOrderService.createBulkCancelOrders(
      bulkCancelledOrdersDto.cancelledOrders,
    );
  }
